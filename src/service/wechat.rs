...
pub async fn login(code: &str) -> Result<LoginResponse> {
    ...
    let response = http::request(Request::new(
        Method::GET, Url::parse_with_params(url.as_str(), query).unwrap()
    ...
    let result: LoginResponse = serde_json::from_str(response.body.as_str()).map_err(|_| Error::WechatHttpResponseParse)?;
