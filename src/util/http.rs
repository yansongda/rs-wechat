...
pub async fn request(request: Request) -> Result<HttpResponse> {
    ...
    let response = client
        .execute(request).await.map_err(|_| Error::Http)?;

    let status = response.status().as_u16();
    let headers = response.headers().iter().map(|(k, v)| (k.to_string(), v.to_str().unwrap().to_string())).collect::<HashMap<String, String>>();
